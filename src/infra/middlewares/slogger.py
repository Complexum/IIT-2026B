...
from src.iit.base.consts import PATH_LOGS
from src.infra.middlewares.color import ColorFormatter
...
class SafeLogger:
    ...
    def __safe_str(self, obj: Any) -> str:
    ...
    def __safe_format(self, *args, **kwargs) -> str:
        """Formatea los argumentos de forma segura."""
        args_str = " ".join(self.__safe_str(arg) for arg in args)
        if kwargs:
            kwargs_str = " ".join(f"{k}={self.__safe_str(v)}" for k, v in kwargs.items())
    ...
    def __setup_logger(self, name: str) -> logging.Logger:
        ...
        # Handler para el archivo "last" — append para no truncar en cada import
        last_file_handler = logging.FileHandler(
            last_log_file, mode="a", encoding="utf-8"
        )
    ...
    def set_log(self, level: int, *args, **kwargs) -> None:
        ...
        message = self.__safe_format(*args, **kwargs)

...
def _build_resource_fig(
    groups_data: dict, rows: int, cols: int, metric: str, metric_label: str, plot_type: str = "line"
):
    ...
    for g_idx, ((ds, pat), df) in enumerate(groups_data.items()):
        ...
        for ci, strat in enumerate(sorted(strats)):
            ...
            if plot_type == "box":
                fig.add_trace(go.Box(
                    y=vals, name=strat, legendgroup=strat,
                    showlegend=(g_idx == 0),
                    marker_color=color, line=dict(color=color),
                    boxmean=True, boxpoints="outliers",
                    hovertemplate=f"<b>{strat}</b><br>{metric_label}=%{{y:.4f}}<extra></extra>",
                ), row=r, col=c)
            else:
    ...
    title_prefix = "Distribución" if plot_type == "box" else metric_label
    fig.update_layout(
        title_text=f"{title_prefix} por estrategia",

...
def hyperfaces(
    ...
    """
    u_idx = np.arange(1 << D, dtype=np.int32)
    bits = (u_idx[:, None] >> np.arange(D, dtype=np.int32)) & 1
    states = np.array(pivot_idx, dtype=np.int32) ^ bits

    idx = (slice(None),) + tuple(states[:, d] for d in range(D))
    sumas: np.ndarray = data_nd[idx].astype(np.float32)

    for d in range(D):
        sh = (N, 1 << (D - d - 1), 2, 1 << d)
        temp = sumas.reshape(sh)
        temp[:, :, 1, :] += temp[:, :, 0, :]
...
class Analytic(SIA, nombre="analytic"):
    ...
    def winner(self, sistema: System) -> tuple[tuple[int, ...], tuple[int, ...]]:
        ...
        # Broadcast del pivote sobre los D ejes del cubo (+ eje N al frente).
        ...
        # Zeta Transform sobre datos normalizados (δ): sumas pueden ser negativas

"""Estrategia AnalyticConcurrent: AnalyticDelta (signed-δ) con evaluación paralela.

Misma estrategia óptima que `analytical` (normalización SIGNADA δ = H − p, pivote→0,
valor absoluto aplicado UNA vez tras promediar), pero la fase de evaluación de máscaras
se reparte en K chunks, un thread por chunk. numpy libera el GIL en ops C → paralelismo real.

Fase 1 — Zeta transform sobre δ: secuencial (las D pasadas son dependientes).
Fase 2 — Evaluación de máscaras: K chunks en paralelo.

Complejidad: O(D·N·2^D) — mismo trabajo que `analytical`, K× más rápido.
Umbral: solo paralela si M ≥ 64 masks (D≥8); por debajo cae al winner serial óptimo.

Modo lazy (D > D_MAX_SOS=25): evita el array (N, 2^D) completo calculando medias de δ
on-the-fly por chunk. Más lento por mask, pero escala a D=26+ sin OOM.
"""
...
# clang + profile + optimal?

# @perfilar
class AnalyticConcurrent(Analytic, nombre="analytic_concurrent"):
    """AnalyticDelta (signed-δ) con evaluación de máscaras en paralelo via ThreadPool."""
    ...
    def winner(self, sistema: System) -> tuple[tuple[int, ...], tuple[int, ...]]:
        ...
        # ── Modo SOS (D ≤ D_MAX_SOS): precomputa δ-sumas, threads las leen ──────
        if D <= D_MAX_SOS:
            sumas = hyperfaces(N, D, delta_nd, pivot_idx)

            def task(chunk: np.ndarray) -> np.ndarray:
                return eval_chunk(chunk, sumas, full_mask, D)

        # ── Modo lazy (D > D_MAX_SOS): cada chunk calcula sus propias medias δ ──
        else:

            def task(chunk: np.ndarray) -> np.ndarray:  # noqa: F811
                return eval_chunk_lazy(chunk, delta_nd, pivot_idx, full_mask, D, N)
        ...
        if D <= D_MAX_SOS:
            ...
            def __derive(mask_a: int) -> tuple[tuple, tuple]:  # noqa: F811
                ...
                va = delta_nd[slc_a].reshape(N, -1).mean(axis=1)
                vb = delta_nd[slc_b].reshape(N, -1).mean(axis=1)
                in_a = np.abs(vb) <= np.abs(va)
                alc = tuple(c.indice for i, c in enumerate(sistema.ncubos) if in_a[i])
                mec = tuple(sistema.dims[d] for d in range(D) if (mask_a >> d) & 1)
                return alc, mec
...
def eval_chunk(
    ...
    """Evalúa f para un chunk de máscaras sobre δ-sumas. Puro numpy → GIL liberado."""
    ...
    sA = sumas[:, chunk_m] / (2.0**sz_A)  # (N, chunk) = mean_A(δ)
    sB = sumas[:, chunk_c] / (2.0**sz_B)  # (N, chunk) = mean_B(δ)

    # C_dist = Σ_i min(|mean_A(δ_i)|, |mean_B(δ_i)|) — abs tras promediar
    return np.minimum(np.abs(sA), np.abs(sB)).sum(axis=0)  # (chunk,)
...
def eval_chunk_lazy(
    ...
    """Evalúa f sobre δ sin sumas precalculadas — para D > D_MAX_SOS (ahorra memoria)."""
    results = np.empty(len(chunk_m), dtype=np.float32)

    for k, mask_a in enumerate(chunk_m):
        mask_b = int(full_mask ^ int(mask_a))
        # mean_A(δ): dims en mask_a libres, resto fijo en pivot (donde δ=0)
        slc_a = tuple(
            [slice(None)]
            + [
        ...
        # mean_B(δ): dims en mask_b libres, resto fijo en pivot
        slc_b = tuple(
            [slice(None)]
            + [slice(None) if (mask_b >> d) & 1 else pivot_idx[d] for d in range(D)]
        )
        mean_a = delta_nd[slc_a].reshape(N, -1).mean(axis=1)
        mean_b = delta_nd[slc_b].reshape(N, -1).mean(axis=1)
        results[k] = float(np.minimum(np.abs(mean_a), np.abs(mean_b)).sum())
    return results
